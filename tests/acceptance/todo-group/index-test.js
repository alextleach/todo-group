...
test('user can see list of todo categories when visiting /todo-groups', function(assert) {
  ...
  andThen(function() {
    ...
    assert.equal(findWithAssert('.todo-list__item').length, 5,
    ...
    assert.equal(findWithAssert('.todo-list__title:first').text().trim(), firstGroup.title,
      'For each "todo-group" pulled from the API, there should be an element with the class' +
      '"todo-list__title" filled with the title of the looped over todo.' +
...
test('user can delete todo-groups when visiting /todo-groups', function(assert) {
  ...
  andThen(function() {
    assert.equal(findWithAssert('.todo-list__item').length, 2,
      'There should be one fewer "todo-list__item" since the todo-group should be deleted' +
